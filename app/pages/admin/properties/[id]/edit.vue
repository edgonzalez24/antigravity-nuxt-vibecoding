  ...
  <div class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <header
      ...
      <div class="space-y-4">
        ...
        <div>
          <h1 class="text-3xl md:text-4xl font-bold text-nordic dark:text-white tracking-tight mb-2">Edit {{
            initialData?.title || 'Property' }}</h1>
      ...
      <div class="flex gap-3 hidden md:flex">
        ...
        <button @click="triggerSave" :disabled="loading || fetching"
    ...
    <AdminPropertyForm v-else ref="formRef" @save="onSave" @cancel="goBack" :loading="loading"
...
const onSave = async (payload) => {
  ...
  try {
    ...
    toastError(`Failed to update property. ${err.message || 'Please try again.'}`)
  } finally {
    loading.value = false
  }
}
