  ...
  <div class="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
    <header class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-200 dark:border-primary/20 pb-8">
      <div class="space-y-4">
        ...
        <div>
          <h1 class="text-3xl md:text-4xl font-bold text-nordic dark:text-white tracking-tight mb-2">Add New Property</h1>
      ...
      <div class="flex gap-3 hidden md:flex">
        ...
        <button @click="triggerSave" :disabled="loading" class="px-5 py-2.5 rounded-lg bg-primary hover:bg-primary/90 text-white font-medium shadow-md transition-all flex items-center gap-2 text-sm disabled:opacity-50">
...
const onSave = async (payload) => {
  ...
  try {
    ...
    alert(`Failed to create property. ${err.message || 'Please try again.'}`)
  } finally {
    loading.value = false
  }
}
