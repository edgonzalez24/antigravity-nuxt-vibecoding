<template>
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    <!-- Modal Overlay -->
    <div class="fixed inset-0 bg-nordic-dark/40 backdrop-blur-sm transition-opacity" @click="closeModal"></div>
    ...
    <main
      ...
      <header
        ...
        <h1 class="text-2xl font-semibold tracking-tight text-nordic-dark">{{ $t('filters.title') }}</h1>
        <button @click="closeModal" class="p-2 rounded-full hover:bg-black/5 transition-colors text-nordic-dark/60">
      ...
      <div class="flex-1 overflow-y-auto hide-scroll p-8 space-y-10">
        ...
        <section>
          ...
          <div class="relative group">
            ...
            <input v-model="filters.location"
              ...
              :placeholder="$t('filters.search_placeholder')" type="text" />
          </div>
        </section>
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div class="space-y-3">
            ...
            <div class="relative">
              <select v-model="filters.type"
                ...
                <option value="sale">{{ $t('filters.for_sale') }}</option>
                <option value="rent">{{ $t('filters.for_rent') }}</option>
              </select>
          ...
          <div class="space-y-4">
            ...
            <div class="flex justify-between items-center">
              <span class="text-sm font-medium text-nordic-dark">{{ $t('filters.bedrooms') }}</span>
              <div class="flex items-center space-x-3 bg-gray-50 rounded-full p-1">
                <button @click="filters.beds > 0 ? filters.beds-- : null" :disabled="filters.beds === 0"
            ...
            <!-- Baths -->
            <div class="flex justify-between items-center">
              <span class="text-sm font-medium text-nordic-dark">{{ $t('filters.bathrooms') }}</span>
              <div class="flex items-center space-x-3 bg-gray-50 rounded-full p-1">
                <button @click="filters.baths > 0 ? filters.baths-- : null" :disabled="filters.baths === 0"
                  class="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center text-nordic-dark/60 hover:text-mosque disabled:opacity-50 transition-colors">
                  <span class="material-icons text-base">remove</span>
                </button>
                <span class="text-sm font-semibold w-4 text-center">{{ filters.baths }}+</span>
                <button @click="filters.baths++"
                  class="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center text-mosque hover:bg-mosque hover:text-white transition-colors">
                  <span class="material-icons text-base">add</span>
                </button>
              </div>
        ...
        <section>
          <label class="block text-xs font-semibold text-nordic-dark/60 uppercase tracking-wider mb-4">
            {{ $t('filters.amenities_features') }}
          </label>
          <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
            <label v-for="amenity in availableAmenities" :key="amenity.value" class="cursor-pointer group relative">
              <input type="checkbox" v-model="filters.amenities" :value="amenity.value" class="peer sr-only" />
              <div
...
const availableAmenities = computed(() => [
  { label: t('filters.pool'), value: 'Pool', icon: 'pool' },
  { label: t('filters.gym'), value: 'Gym', icon: 'fitness_center' },
  { label: t('filters.parking'), value: 'Parking', icon: 'local_parking' },
  { label: t('filters.ac'), value: 'AC', icon: 'ac_unit' },
  { label: t('filters.wifi'), value: 'Wifi', icon: 'wifi' },
  { label: t('filters.patio'), value: 'Patio', icon: 'deck' }
])
